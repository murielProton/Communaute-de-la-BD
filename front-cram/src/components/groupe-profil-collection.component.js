...
export default class ListeCollectionMembreGroupe extends Component {
    ...
    componentDidMount() {//Pour récupérer les données liste de la base de données, la méthode de cycle de vie componentDidMount est ajoutée:
        ...
        api.get('collection/liste/' + this.props.match.params.pseudo)
            .then(response => {
    ...
    render() {
        return (
            <div>
                <h3>Collection de bande-dessinées de {this.props.match.params.pseudo}</h3>
                {this.state.bedes.map(bede => (
                        <div>
                            ...
                            <div className="card">
                                ...
                                <p><Link to={"/collection/bd/detail/" + detail_bede._id}> Voir plus</Link></p>
                                <p></p>
                            </div>
                             ))}
                        </div>
