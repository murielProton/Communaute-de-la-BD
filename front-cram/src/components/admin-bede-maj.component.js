...
export default class AdminMajBede extends Component {
    ...
    componentDidMount() { // Permet de récupérer les données de la bédé à modifier afin de les avoir dans le formulaire en valeurs par défaut
        api.get('admin/bede/prendrecontenu/' + this.props.match.params.id)
            .then(res => {
    ...
    onSubmit(e) {
        ...
        api.post('admin/bede/maj/' + this.props.match.params.id, majBede)
            .then(response => {
    ...
    render() {
        if (this.state.redirection) {
            //Redirect to the page
               // return <Redirect to={`/member/${localStorage.getItem('username')}`}/>;
