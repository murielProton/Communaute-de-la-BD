...
export default class AdminBannirMembre extends Component {
    constructor(props){
        super(props);
    ...
    componentDidMount() {
        api.get('admin/membre/bannir/' + this.props.match.params.id)
            .then(response => {
                if (response.status === 200 && response !== null) {
                    ...
                    console.log('Echec du banissement');
                }
            })
            .catch(err => {
                console.log(err)
    ...
    render(){
        ...
        return(
        <div style={{marginTop: 60}}><h2>Sur la page bannir un membre !!</h2></div>
        )
    }
}