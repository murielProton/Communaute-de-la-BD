...
export default class AdminListeBedes extends Component {
                ...
                <table className="table table-striped" style={{ marginTop: 20 }} >
                    ...
                    {this.state.bedes.map((bede) =>//nous parcourons la liste des tâches à faire en utilisant la fonction map. Chaque élément de tâche est généré à l'aide du composant Todo qui n'est pas encore implémenté. L'élément de tâche actuel est affecté à la propriété de tâche de ce composant.
                        <tr>
                            ...
                            <td>{bede.tome}</td>
                            <td>{bede.titre}</td>
                            <td><Link to={"/admin/bd/detail/" + bede._id} className="btn btn-primary">Voir plus</Link></td>
                            <td><Link to={"/admin/bd/maj/" + bede._id} className="btn btn-success">Modifier</Link></td>
                            <td><Link to={"/admin/bd/supprimer/" + bede._id} className="btn btn-danger">Supprimer</Link></td>
                        </tr>
                    )}
                    </tbody>
                </table>
