...
export default class AdminListeDiscussion extends Component {
                ...
                <table className="table table-striped" style={{ marginTop: 30 }} >
                    ...
                    {discussions.map(discussion =>( //nous parcourons la liste des discussion en cours en utilisant la fonction de carte. Chaque élément de discussion est généré à l'aide du composant  qui n'est pas encore implémenté. 
                        <tr>
                            ...
                            <td key={discussion.uid}> <Link to={"/admin/discussion/detail/"+discussion._id}>{discussion.titre}</Link></td>
                            <Link to={"/admin/discussion/maj/"+ discussion._id} className="btn btn-info">Modifier le titre de la discussion</Link>
